...
!function(app){"use strict";app.registerModule("products",["core"]),app.registerModule("products.admin",["core.admin"]),app.registerModule("products.admin.routes",["core.admin.routes"]),app.registerModule("products.services"),app.registerModule("products.routes",["ui.router","core.routes","products.services"])}(ApplicationConfiguration);
...
!function(){"use strict";function menuConfig(Menus){Menus.addSubMenuItem("topbar","admin",{title:"Manage Products",state:"admin.products.list"})}angular.module("products.admin").run(menuConfig),menuConfig.$inject=["menuService"]}();
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("admin.products",{abstract:!0,url:"/products",template:"<ui-view/>"}).state("admin.products.list",{url:"",templateUrl:"/modules/products/client/views/admin/list-products.client.view.html",controller:"ProductsAdminListController",controllerAs:"vm",data:{roles:["admin"]}}).state("admin.products.create",{url:"/create",templateUrl:"/modules/products/client/views/admin/form-product.client.view.html",controller:"ProductsAdminController",controllerAs:"vm",data:{roles:["admin"]},resolve:{productResolve:newProduct}}).state("admin.products.edit",{url:"/:productId/edit",templateUrl:"/modules/products/client/views/admin/form-product.client.view.html",controller:"ProductsAdminController",controllerAs:"vm",data:{roles:["admin"]},resolve:{productResolve:getProduct}})}function getProduct($stateParams,ProductsService){return ProductsService.get({productId:$stateParams.productId}).$promise}function newProduct(ProductsService){return new ProductsService}angular.module("products.admin.routes").config(routeConfig),routeConfig.$inject=["$stateProvider"],getProduct.$inject=["$stateParams","ProductsService"],newProduct.$inject=["ProductsService"]}();
!function(){"use strict";function menuConfig(menuService){menuService.addMenuItem("topbar",{title:"Products",state:"products",type:"dropdown",roles:["*"]}),menuService.addSubMenuItem("topbar","products",{title:"List Products",state:"products.list",roles:["*"]})}angular.module("products").run(menuConfig),menuConfig.$inject=["menuService"]}();
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("products",{abstract:!0,url:"/products",template:"<ui-view/>"}).state("products.list",{url:"",templateUrl:"/modules/products/client/views/list-products.client.view.html",controller:"ProductsListController",controllerAs:"vm",data:{pageTitle:"Products List"}}).state("products.view",{url:"/:productId",templateUrl:"/modules/products/client/views/view-product.client.view.html",controller:"ProductsController",controllerAs:"vm",resolve:{productResolve:getProduct},data:{pageTitle:"product {{ productResolve.title }}"}})}function getProduct($stateParams,ProductsService){return ProductsService.get({productId:$stateParams.productId}).$promise}angular.module("products.routes").config(routeConfig),routeConfig.$inject=["$stateProvider"],getProduct.$inject=["$stateParams","ProductsService"]}();
!function(){"use strict";function ProductsListController(ProductsService){this.products=ProductsService.query()}angular.module("products").controller("ProductsListController",ProductsListController),ProductsListController.$inject=["ProductsService"]}();
!function(){"use strict";function ProductsController($scope,product,Authentication){var vm=this;vm.product=product,vm.authentication=Authentication}angular.module("products").controller("ProductsController",ProductsController),ProductsController.$inject=["$scope","productResolve","Authentication"]}();
!function(){"use strict";function ProductsService($resource,$log){function createOrUpdate(product){function onSuccess(product){}function onError(errorResponse){handleError(errorResponse.data)}return product._id?product.$update(onSuccess,onError):product.$save(onSuccess,onError)}function handleError(error){$log.error(error)}var Product=$resource("/api/products/:productId",{productId:"@_id"},{update:{method:"PUT"}});return angular.extend(Product.prototype,{createOrUpdate:function(){return createOrUpdate(this)}}),Product}angular.module("products.services").factory("ProductsService",ProductsService),ProductsService.$inject=["$resource","$log"]}();
...
!function(){"use strict";function ProductsAdminListController(ProductsService){this.products=ProductsService.query()}angular.module("products.admin").controller("ProductsAdminListController",ProductsAdminListController),ProductsAdminListController.$inject=["ProductsService"]}();
!function(){"use strict";function ProductsAdminController($scope,$state,$window,product,Authentication,Notification){function remove(){$window.confirm("Are you sure you want to delete?")&&vm.product.$remove(function(){$state.go("admin.products.list"),Notification.success({message:'<i class="glyphicon glyphicon-ok"></i> product deleted successfully!'})})}function save(isValid){function successCallback(res){$state.go("admin.products.list"),Notification.success({message:'<i class="glyphicon glyphicon-ok"></i> product saved successfully!'})}function errorCallback(res){Notification.error({message:res.data.message,title:'<i class="glyphicon glyphicon-remove"></i> product save error!'})}if(!isValid)return $scope.$broadcast("show-errors-check-validity","vm.form.productForm"),!1;vm.product.createOrUpdate().then(successCallback).catch(errorCallback)}var vm=this;vm.product=product,vm.authentication=Authentication,vm.form={},vm.remove=remove,vm.save=save}angular.module("products.admin").controller("ProductsAdminController",ProductsAdminController),ProductsAdminController.$inject=["$scope","$state","$window","productResolve","Authentication","Notification"]}();
